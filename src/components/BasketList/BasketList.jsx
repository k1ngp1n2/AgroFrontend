...
export default class BasketList extends PureComponent {
  ...
  render() {
    ...
    const sum = basketItems.cart_items
      .map(item => {
    ...
    return (
      <div className='basket_list'>
        ...
        {basketItems.cart_items.map((item, idx) => {
          return (
            <p className='basket_item' key={idx}>
              ...
              />
              <span className='item_name'>{item.cart_item.product.name}</span>
              ...
              <span className='item_quantity'>{item.cart_item.quantity}</span>
              <Button
              ...
              <span className='item_full_price'>
                {(
                  item.cart_item.product.price * item.cart_item.quantity
                ).toLocaleString('ru')}{' '}
                руб.
              </span>
              <Button
        ...
        <p className='basket_total'>
          Общая стоимость доставки товаров в корзине:{' '}
          {deliveryCost.toLocaleString('ru')} руб.
        </p>
        <p className='basket_total'>
        ...
        <p className='basket_total'>
          ...
          {(deliveryCost + sum).toLocaleString('ru')} руб.
        </p>
      </div>
    );
  }
