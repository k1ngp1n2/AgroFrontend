...
export default class ProfileProfit extends PureComponent {
  ...
  sendMoney = () => {
    ...
    fetch(`${serverAddress}/api/member/transactions`, {
      ...
      .then(res => res.json())
      .then(res => {
        if (res.result.transaction.id !== '') itemHandle('profile_account');
        else
      ...
      return (
        <div className='details_for_return seller_items'>
          ...
          <input
            ...
            onChange={this.handleChange}
          />
          <label className='item_account_number' htmlFor='label_account_number'>
          ...
          <input
            ...
            onChange={this.handleChange}
          />
          <label className='item_bank' htmlFor='label_bank'>
            Наименование банка
          </label>
          <input
            type='text'
