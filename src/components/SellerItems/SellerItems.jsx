...
export default class SellerItems extends PureComponent {
  ...
  render() {
    ...
    if (error) {
      subcontent = (
        <p className='load_info sell_items_content'>Ошибка: {error.message}</p>
      );
    } else if (!itemsLoaded) {
      subcontent = (
        <p className='load_info sell_items_content'>
          Пожалуйста, подождите, идет загрузка страницы
        </p>
      );
    } else {
      if (
        sellerItems === undefined ||
        sellerItems.length === 0 ||
        sellerItems.products === undefined ||
        sellerItems.products.length === 0
      ) {
        subcontent = (
          <div className='load_info sell_items_content'>
            <div />
            <p>
              Вы можете добавить товары на продажу с помощью кнопки{' '}
              {newSellButton.name}
            </p>
          </div>
        );
        ...
        subcontent = (
          <div className='sell_items_content'>
